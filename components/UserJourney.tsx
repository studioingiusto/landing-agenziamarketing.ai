...
import { Globe, Search, FileText, Key, BarChart, FileCode, ArrowRight } from 'lucide-react';
...
export function UserJourney() {
  ...
  const steps: JourneyStep[] = [
    {
      id: 1,
      title: "Analizza il tuo sito e i tuoi competitor",
      description: "L’AI scansiona il tuo sito, identifica errori SEO e suggerisce miglioramenti.",
      icon: <Globe className="w-6 h-6 text-[#9c55ff]" />,
      position: 'left'
    },
    {
      id: 2,
      title: "Scopri le migliori keyword in pochi secondi",
      description: "Ottieni keyword strategiche con volumi di ricerca e difficoltà aggiornati",
      icon: <FileText className="w-6 h-6 text-[#9c55ff]" />,
      position: 'right'
    },
    {
      id: 3,
      title: "Genera contenuti SEO-friendly automaticamente",
      description: "Scrivi articoli, schede prodotto e post con testi ottimizzati per Google.",
      icon: <BarChart className="w-6 h-6 text-[#9c55ff]" />,
      position: 'left'
    },
    {
      id: 4,
      title: "Pubblicazione Automatizzata",
      description: "Pubblica contenuti SEO-ottimizzati direttamente sul tuo CMS (WordPress, Shopify, etc.) con un solo clic, risparmiando ore di lavoro.",
      ...
      (entries) => {
        entries.forEach((entry) => {
          ...
          if (entry.isIntersecting) {
            setActiveSteps((prev) => 
              prev.includes(stepId) ? prev : [...prev, stepId].sort((a, b) => a - b)
            );
          }
        });
      },
  ...
  return (
    <div className="w-full max-w-5xl mx-auto py-24" ref={containerRef}>
      ...
      <div className="relative">
        ...
        <div className="absolute left-1/2 top-0 bottom-0 w-1 bg-gradient-to-b from-[#9c55ff]/0 via-[#9c55ff] to-[#9c55ff]/0 transform -translate-x-1/2 z-0">
          <div 
            ...
            style={{ 
              height: `${Math.min(100, (activeSteps.length / steps.length) * 100)}%`,
        ...
        <div className="relative z-10">
          {steps.map((step) => (
            <div 
              ...
              } ${step.id % 2 === 0 ? 'md:flex-row-reverse' : 'md:flex-row'} relative`}
              data-step-id={step.id}
            >
              {/* Large background step number */}
              <div 
                className={`absolute -z-10 select-none pointer-events-none
                  ${step.position === 'left' ? 'right-[20%] md:right-[25%]' : 'left-[20%] md:left-[25%]'}
                  ${step.id % 2 === 0 ? 'md:left-[25%] md:right-auto' : 'md:right-[25%] md:left-auto'}
                  top-1/2 -translate-y-1/2
                `}
              >
                <span 
                  className={`text-[150px] md:text-[220px] lg:text-[280px] font-bold leading-none inline-block
                  ${activeSteps.includes(step.id) ? 'text-stroke-active' : 'text-stroke-inactive'}`}
                  style={{ 
              ...
              <div 
                ...
                } ${step.id % 2 === 0 ? 'md:ml-10 md:mr-20 md:text-right !pl-16' : 'md:mr-10 md:ml-20 md:text-left !pr-16'}
              ...
              >
                ...
                } ${step.id % 2 === 0 ? 'md:justify-end' : 'md:justify-start'} mb-4`}>
                  <div className={`p-2 bg-[#2a193c]/80 rounded-full ${
                    step.position === 'left' ? 'order-2 ml-3' : 'order-1 mr-3'
                  } ${step.id % 2 === 0 ? 'md:order-2 md:ml-3 md:mr-0' : 'md:order-1 md:mr-3 md:ml-0'}`}>
                    {step.icon}
                  </div>
                  <h3 className={`text-white text-xl font-medium font-title ${
